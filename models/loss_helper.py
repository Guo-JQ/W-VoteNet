...
DATASET_CONFIG_scannet = ScannetDatasetConfig()
DATASET_CONFIG_sunrgbd = SunrgbdDatasetConfig()
...
def compute_cls_loss(end_points):
    batch_size = end_points['seed_seg_point'].shape[0]
    num_seed = end_points['seed_seg_point'].shape[1]  # B,num_seed,3

    seed_seg_point = end_points['seed_seg_point']  # B,num_seed*vote_factor,3
    seed_inds = end_points['seed_inds'].long()  # B,num_seed in [0,num_points-1]
    seed_seg_point = seed_seg_point.squeeze(dim=-1)

    seed_gt_votes_mask = torch.gather(end_points['vote_label_mask'], 1, seed_inds)

    criterion = nn.BCELoss()
    loss = criterion(seed_seg_point.float(), seed_gt_votes_mask.float())
...
def compute_vote_loss(end_points):
    ...
    # background_gt_votes_mask = 1 - seed_gt_votes_mask
    # backgroundkweight = background_gt_votes_mask / (torch.sum(background_gt_votes_mask) + 1e-6)
    # non_vote_loss = torch.sum(
    #     torch.norm(vote_xyz.view(batch_size * num_seed, 3), dim=1) * backgroundkweight.view(batch_size * num_seed))

    return vote_loss


def compute_objectness_loss_cluster(end_points):
    ...
    cluster_vote_xyz = end_points['cluster_vote_xyz']
    gt_center = end_points['center_label'][:, :, 0:3]
    np.set_printoptions(threshold=np.inf)

    B = gt_center.shape[0]
    K = aggregated_vote_xyz.shape[1]
    K_cluster = cluster_vote_xyz.shape[1]
    dist1, ind1, dist2, _ = nn_distance(aggregated_vote_xyz, gt_center)  # dist1: BxK, dist2: BxK2
    cdist1, cind1, cdist2, _ = nn_distance(cluster_vote_xyz, gt_center)  # dist1: BxK, dist2: BxK2
    # Generate objectness label and mask
    # objectness_label: 1 if pred object center is within NEAR_THRESHOLD of any GT object
    # objectness_mask: 0 if pred object center is in gray zone (DONOTCARE), 1 otherwise
    euclidean_dist1 = torch.sqrt(dist1 + 1e-6)
    ceuclidean_dist1 = torch.sqrt(cdist1 + 1e-6)
    objectness_label = torch.zeros((B, K), dtype=torch.long).cuda()
    cluster_label = torch.zeros((B, K_cluster), dtype=torch.long).cuda()
    objectness_mask = torch.zeros((B, K)).cuda()
    objectness_label[euclidean_dist1 < NEAR_THRESHOLD] = 1
    cluster_label[ceuclidean_dist1 < NEAR_THRESHOLD] = 1
    ...
    cobject_assignment = cind1  # (B,K) with values in 0,1,...,K2-1

    return objectness_loss, objectness_label, objectness_mask, object_assignment, cluster_label, cobject_assignment
...
def compute_objectness_loss(end_points):
    ...
    criterion = nn.CrossEntropyLoss(torch.Tensor(OBJECTNESS_CLS_WEIGHTS).cuda())
    objectness_loss = criterion(objectness_scores.transpose(2, 1), objectness_label)
...
def compute_box_and_sem_cls_loss(end_points, config):
    ...
    # print("Cluster label:",sem_cls_label)
...
#
#     # cls_loss = compute_cls_loss(end_points)
...
#
#     # Final loss function
#     # loss = vote_loss + 0.5 * objectness_loss + box_loss + 0.1 * sem_cls_loss+0.2*cls_loss
...
#     end_points['obj_acc'] = obj_acc  # ori: obj_acc
#
...
def get_loss(end_points, config):
    ...
    if 'cluster_vote_xyz' in end_points.keys():
        objectness_loss, objectness_label, objectness_mask, object_assignment, cluster_label, cluster_assignment = \
            compute_objectness_loss_cluster(end_points)
        end_points['cluster_label'] = cluster_label
        end_points['cluster_assignment'] = cluster_assignment
    else:
        objectness_loss, objectness_label, objectness_mask, object_assignment = \
            compute_objectness_loss(end_points)
    end_points['objectness_loss'] = objectness_loss
    ...
    # Relation cls loss
    if 'cluster_vote_xyz' in end_points.keys():
        rn_cls_losses = compute_multi_rn_cls_loss_cluster(end_points)
    else:
        rn_cls_losses = compute_multi_rn_cls_loss(end_points)
    end_points['rn_cls_loss'] = rn_cls_losses[0]

    '''ori'''
    loss = vote_loss + 0.5 * objectness_loss + box_loss + 0.1 * sem_cls_loss + 0.1 * rn_cls_losses[1] + 0.1 * rn_cls_losses[2] + 0.2 * cls_loss
    ...
    obj_pred_acc = torch.sum((obj_pred_val == objectness_label.long()).float()) / (
            objectness_label.shape[0] * objectness_label.shape[1])

    end_points['obj_acc'] = obj_pred_acc

    return loss, end_points


def compute_multi_rn_cls_loss_cluster(end_points):
    relation_type = end_points['relation_type']
    object_assignment = end_points['object_assignment']
    objectness_label = end_points['objectness_label'].float()
    cluster_assignment = end_points['cluster_assignment']
    cluster_label = end_points['cluster_label'].float()
    ...
    bs = objectness_label.shape[0]
    roi_num = objectness_label.shape[1]
    pair_num = cluster_label.shape[1]

    _mask_i = objectness_label.unsqueeze(2).repeat(1, 1, pair_num).view(bs, roi_num * pair_num)
    _mask_j = cluster_label.unsqueeze(1).repeat(1, roi_num, 1).view(bs, roi_num * pair_num)

    rn_mask = (_mask_i.mul(_mask_j)).float()

    if 'semantic_relation' in relation_type:
        rn_scores_0 = end_points['rn_logits_0']
        proposal_labels = torch.gather(end_points['sem_cls_label'], 1,
                                       object_assignment)  # object_assignment的shape是多少 (bs, num_proposal) I GUESS
        cluster_cls = torch.gather(end_points['sem_cls_label'], 1, cluster_assignment)
...
def compute_multi_rn_cls_loss(end_points):
    ...
    _mask_i = objectness_label.unsqueeze(2).repeat(1, 1, pair_num).view(bs, roi_num * pair_num)
    if 256 in nearest_n_index:
        nearest_n_index_reshape2 = nearest_n_index.reshape(bs, roi_num * pair_num)
        nearest_n_index_reshape[nearest_n_index_reshape == 256] = 0
    _mask_j = objectness_label.index_select(1, nearest_n_index_reshape)

    _mask_j = _mask_j[
        torch.arange(bs).cuda().view(bs, -1), torch.arange(bs * roi_num * pair_num).view(bs, -1)]  # (8,256*256)

    if 256 in nearest_n_index:
        _mask_j[nearest_n_index_reshape2 == 256] = 0
    rn_mask = (_mask_i.mul(_mask_j)).float()
...
def get_obbs(end_points):
    ...
    if end_points['model'] == 'scannet':
        for i in range(bsize):
            for j in range(num_proposal):
                heading_angle = DATASET_CONFIG_scannet.class2angle(
                    pred_heading_class[i, j].detach().cpu().numpy(), pred_heading_residual[i, j].detach().cpu().numpy())
                box_size = DATASET_CONFIG_scannet.class2size( \
                    int(pred_size_class[i, j].detach().cpu().numpy()), pred_size_residual[i, j].detach().cpu().numpy())
                corners_3d_upright_camera = get_3d_box(box_size, heading_angle, pred_center_upright_camera[i, j, :])
                pred_corners_3d_upright_camera[i, j] = corners_3d_upright_camera
    if end_points['model'] == 'sunrgbd':
        for i in range(bsize):
            for j in range(num_proposal):
                heading_angle = DATASET_CONFIG_sunrgbd.class2angle(
                    pred_heading_class[i, j].detach().cpu().numpy(), pred_heading_residual[i, j].detach().cpu().numpy())
                box_size = DATASET_CONFIG_sunrgbd.class2size( \
                    int(pred_size_class[i, j].detach().cpu().numpy()), pred_size_residual[i, j].detach().cpu().numpy())
                corners_3d_upright_camera = get_3d_box(box_size, heading_angle, pred_center_upright_camera[i, j, :])
                pred_corners_3d_upright_camera[i, j] = corners_3d_upright_camera
...
def compute_spatial_relation(obbs_i, obbs_j, relation_type=1):
    ...
    obbs_i_repeat = obbs_i.unsqueeze(2).repeat(1, 1, 8, 1)
    obbs_j_repeat = obbs_j.unsqueeze(1).repeat(1, 8, 1, 1)
    dist = (obbs_i_repeat - obbs_j_repeat).pow(2)
    dist = torch.sum(dist, -1)
    dist = dist.reshape(bs * proposal_num, 8 * 8)
