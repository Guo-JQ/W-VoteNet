...
class SRModule(nn.Module):
    def __init__(self, relation_pair=3, relation_type='semantic_relation', random=False):
        super().__init__()
    ...
    def forward(self, feature, end_points):
        ...
        for i, num in enumerate(sum_one):
        ...
        _feature_i = feature.unsqueeze(2).repeat(
            (1, 1, self.max_paris, 1))  # [B, proposal_num, self.max_pairs, feat_in]
        _range_for_bs = torch.arange(bs).unsqueeze(1).repeat((1, num_proposal * self.max_paris)).reshape(
            (bs * num_proposal * self.max_paris))
        ...
        if 'semantic_relation' in self.relation_type:
